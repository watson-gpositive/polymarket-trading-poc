...
export const config = {
  ...
  fEnabled: (process.env.F_ENABLED ?? 'true') === 'true',
  fHoursToEventMax: toNum(process.env.F_HOURS_TO_EVENT_MAX, 48),
  fMinSpreadCents: toNum(process.env.F_MIN_SPREAD_CENTS, 3),
  fTargetQty: toNum(process.env.F_TARGET_QTY, 5),
  gEnabled: (process.env.G_ENABLED ?? 'true') === 'true',
  gMinSimilarityKeyLen: toNum(process.env.G_MIN_SIMILARITY_KEY_LEN, 12),
  gMinDivergenceCents: toNum(process.env.G_MIN_DIVERGENCE_CENTS, 8),
  gTargetQty: toNum(process.env.G_TARGET_QTY, 5),
